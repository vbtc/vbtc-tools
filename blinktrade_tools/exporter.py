...
def send_msg(msg):
    ...
    if sys.version_info[0] == 2:
        ...
        signature = hmac.new(
            secret.encode(), nonce.encode(), digestmod=hashlib.sha256
        ).hexdigest()
    headers = {
        ...
        for response in api_responses:
            if response["MsgType"] == "U35":
                ...
                for ledger_array_record in ledger_list:
                    ledger_record = dict(
                        list(zip(response["Columns"], ledger_array_record))
                    )
...
def generate_trade_record(record_0, record_1, balance_dict):
    ...
    if (
        record_0["Operation"] == "C"
        and record_0["Currency"] == "BTC"
        and record_1["Operation"] == "D"
    ):
        trade_type = "BUY"
        ...
        record_0["Operation"] == "D"
        and record_0["Currency"] == "BTC"
        and record_1["Operation"] == "C"
    ):
        trade_type = "SELL"
        exchange_rate = record_1["Amount"] / record_0["Amount"] * 1e8
        amount_btc = record_0["Amount"]
        ...
        record_1["Operation"] == "C"
        and record_1["Currency"] == "BTC"
        and record_0["Operation"] == "D"
    ):
        trade_type = "BUY"
        ...
        record_1["Operation"] == "D"
        and record_1["Currency"] == "BTC"
        and record_0["Operation"] == "C"
    ):
        trade_type = "SELL"
        exchange_rate = record_0["Amount"] / record_1["Amount"] * 1e8
        amount_btc = record_1["Amount"]
    ...
    description = "%s %s @ %s %s" % (
        trade_type,
        "BTC {:,.8f}".format(amount_btc / 1e8),
        fiat_currency,
        "{:,.2f} per BTC".format(exchange_rate / 1e8),
    )
...
def generate_record(record_0, balance_dict):
    ...
    record = {
        ...
        "Total": record_0["Amount"] / 1e8
        if record_0["Operation"] == "C"
        else record_0["Amount"] / 1e8 * -1,
    }
...
def main():
    ...
    while ledger_index < len(ledger_response):
        ...
        if ledger_item["Description"] == "T":
            ...
            formatted_record = generate_trade_record(
                ledger_item, next_ledger_item, balance_dict
            )
